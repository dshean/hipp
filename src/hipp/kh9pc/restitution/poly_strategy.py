...
class PolyStrategy(RestitutionStrategy):
    ...
    def extended_window(self, rail_px: int = 1600) -> tuple:
        """(extended Transformation, sidecar dict) for ``transform_extended`` -- no warp.

        A strategy unpickled with its transformation already cached never ran
        ``_compute_transformation`` in this process, so the canvas-space geometry it stores
        (``_top_dst_`` ...) is absent (job 25158475, 2026-09-19). Recompute it here and REFUSE
        if the recomputed crop_offset / output_size differ from the cached ones: the sidecar
        must describe the warp that was (or will be) written, not a different one.
        """
        import dataclasses
        tf = self.transformation_
        if not hasattr(self, "_top_dst_"):
            cached = (tuple(int(v) for v in tf.crop_offset), tuple(int(v) for v in tf.output_size))
            fresh = self._compute_transformation()
            got = (tuple(int(v) for v in fresh.crop_offset), tuple(int(v) for v in fresh.output_size))
            if got != cached:
                raise RuntimeError(
                    "%s: recomputed transformation %s != pickled %s -- the environment (POLY_EDGE_CLASS_POLICY, "
                    "KH9_IMAGE_HEIGHT_PX, KH9_SECTOR_WIDTH_PX) is not the one the block was restituted under"
                    % (getattr(self, "raster_filepath_", "?"), got, cached))
        rail_px = int(rail_px)
    ...
    def transform_extended(self, output_path: str | Path, rail_px: int = 1600) -> dict:
        ...
        rows with a uniform period: an easier detection and a check on the restitution itself
        (a bent row or a stepped period = a bad seam or edge model). Sidecar: ``<output>.json``
        (see ``extended_window``)."""
